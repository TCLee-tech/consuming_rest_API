...
public class AzureSentimentService {
	...
	//@Value("${AZURE_API_KEY}") Use this if API_Key added as environmental variable
	private String azureApiKey = "Insert API_Key from Azure here";
	...
	public SentimentAnalysis requestSentimentAnalysis(String text, String language) throws IOException, InterruptedException {
		...
		TextDocument document = new TextDocument("1", text, language); // replace "text" if you want to test certain phrases
		TextAnalyticsRequest requestBody = new TextAnalyticsRequest();
		requestBody.getDocuments().add(document);
		
		String endpoint = AZURE_ENDPOINT + "/text/analytics/v3.1-preview.4/sentiment";
